  ...
  if (pagina == 1) {
    ...
    if (cambio == 0) {
      ...
      if (rangoAUX > 1) {
        ...
        rango2 = map(rango, 50, 600, 150, 500);
      }
      delay(100);
...
void CalcularTiempos() {
  ...
  float gain_desc = 6.8;
  float gain_asc = 6.48;
  float t_desc = 62 * (float)rango3  * ((float)P * gain_desc + 80);
...
void Muestreo() {
  ...
  char str8[8];
  char str9[8];
  itoa(cont + 20, str1, 10);
  itoa(lectura * 10 - 130, str2, 10);
  itoa(voltio, str3, 10);
  itoa(rango, str4, 10);
  itoa(RPM, str5, 10);
  itoa(lectura * 1.5 + 120, str6, 10);
  // itoa(senal, str6, 10);
  itoa(signalvalue, str7, 10);
  itoa(ratio, str8, 10);
  itoa(frecuencia * 504, str9, 10);//288
  //Serial.println(str);
...
void Presion() {
  if (scale.is_ready()) {
    ...
    lectura = map(reading, 820000, 3200000, 5, 50);
  } else {

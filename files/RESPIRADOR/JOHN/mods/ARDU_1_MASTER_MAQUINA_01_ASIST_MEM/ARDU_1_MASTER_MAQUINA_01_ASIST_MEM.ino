...
#define RELAY1 3 //3 desconectar cargador
#define RELAY2 8 //8 CHOOSE POWER SOURCE
#define LED1 11
#define LED2 9
#define LED3 7
#define LED4 10
...
#define RxD 5   //5                                                                        // Usado para conexion rs232 virtual.
#define TxD 6  //6
int cont = 0;
int gate = 0;
int contador = 0;
...
void setup()
{
  ...
  if(readLongFromEEPROM(100)>0){
    total_ciclos = readLongFromEEPROM(100);
  } else {
    total_ciclos = 10;
  }
  
  if (total_ciclos > 2000000) {
    myNextion.sendCommand("page 8");
    while (true) {
      myNextion.setComponentValue("n0", total_ciclos);
      digitalWrite(4, HIGH);
      delay(1000);
      digitalWrite(4, LOW);
      delay(1000);
...
void ComunicarPantalla() {
  ...
  if (pagina == 3) {//PARAMETROS
    ...
    if (cambio == 0) {
      myNextion.setComponentValue("ciclos", total_ciclos); delay(2);
...
void Muestreo() {
  ...
  char buf9[30];
  char buf10[30];
  const char *first1 = "add 22,1,";//GRAF. VOL
  ...
  char str10[8];
  itoa(cont, str1, 10);
  itoa(presion_actual * 10, str2, 10);
  itoa(voltio, str3, 10);
  itoa(rango, str4, 10);
  itoa(RPM, str5, 10);
  itoa(lectura * 2, str6, 10);
  // itoa(senal, str6, 10);
  ...
  strcpy(buf9, first9);
  strcat(buf9, second9);
  strcpy(buf10, first10);
  strcat(buf10, second10);
  //Serial.println(buf1);
  ...
  myNextion.sendCommand(buf10);
  if (presion_actual > p_max) {
    interrupts();
  ...
  if (switche > 100) {
    if (modo == 1) {
      ...
      ciclos++;
      if (ciclos % 50 == 0) {
        total_ciclos = total_ciclos + 50;
        writeLongIntoEEPROM(100, total_ciclos);
      }
      digitalWrite(4, LOW);
    } else if (modo == 2) {
      TE = timecount;

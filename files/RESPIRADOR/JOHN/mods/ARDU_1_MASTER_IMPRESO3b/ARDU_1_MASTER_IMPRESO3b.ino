...
#define RELAY1 8 //desconectar cargador
#define RELAY2 3 //CHOOSE POWER SOURCE
//CAMBIAR A4 POR D8
//CAMBIAR A5 POR D3
#define LED1 11
#define LED2 9
...
SoftwareSerial nextion2(TxD2, RxD2);
//SoftwareSerial portOne(10, 11);
Nextion myNextion(nextion, 57600);
Nextion myNextion2(nextion2, 57600);
...
void setup()
{
  ...
  pinMode(ENLACE, INPUT);//A2 MASTER INPUT -  SLAVE A2 OUT for senal
  ...
  //myNextion2.sendCommand("bauds=57600");
  attachInterrupt(0, ContarPulsos, RISING); //(Interrupción 0(Pin2),función,Flanco de subida)
  t0 = millis();
  Serial.begin(38400);
  // portOne.begin(38400);
  if (myNextion.init()) {
    Serial.println(F("Nextion1 Ready..."));
  } else {
    Serial.println(F("Nextion not responding..."));
  }
  if (myNextion2.init()) {
    Serial.println(F("Nextion2 Ready..."));
  } else {
    Serial.println(F("Nextion not responding..."));
  }
...
void CalcularPausas() {
  ...
  t_pausa1 = abs((t_base * (float)ratio) / ((float)rango2 * 46)) - constante1;

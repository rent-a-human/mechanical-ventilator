...
double cuentatela = 0;
double avanceVuelta = 21.6;
double longitudTotal = 0;
double longitudTotalenM = 0;
long longitudDeseada = 0;
long longitudDeseadaAUX = 0;
...
void setup() {
  ...
  delay(300);
  myNextion.setComponentValue("n4", 0);
  myNextion.setComponentValue("n3", 100);
  myNextion.setComponentValue("n2", 0);
  longitudDeseada = myNextion.getComponentValue("n3");
}
  ...
  if (pagina == 1) {
    ...
    if (bt1p1 == 1) {
      ...
      if (pedal < 200 && sensor1 < 200) {
        ...
        Main();
        ciclos++;
        longitudDeseadaAUX = myNextion.getComponentValue("n3");
        if (longitudDeseadaAUX > 0) {
          longitudDeseada = longitudDeseadaAUX;
          if (longitudDeseada <= longitudTotalenM) {
            digitalWrite(BUZZER, HIGH);
...
void detectChanges() {
  sensor2 = analogRead(SENSOR2);//NO detectando sensor = 333, DETECTANDO <10
  if (sensor2 < 100) {
    detectando = true;
  } else {
    detectando = false;
  }
  if (detectando == true && previousState == true) {
    return;
    //no hacer nada
  } else if (detectando == true && previousState == false) {
    cuentatela++;
    longitudTotal = cuentatela * avanceVuelta;
    longitudTotalenM = longitudTotal / 100; //160mm = 0.16m en pantalla 1 = 0.1
    myNextion.setComponentValue("n4", longitudTotalenM);
    myNextion.setComponentValue("n2", longitudTotalenM);
    previousState = true;
  } else if (detectando == false && previousState == true) {
    previousState = false;
  } else if (detectando == false && previousState == false) {
    //no hacer nada
  }
}
...
void Main() {
  //digitalWrite(BUZZER, HIGH);
  diametroAUX = myNextion.getComponentValue("n0");
  if (diametroAUX > 1) {
    diametro = diametroAUX;
  }
  pot1 = analogRead(POT1);
